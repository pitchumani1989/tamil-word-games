...
var xWords = {
	...
	AddWord: function(newWord, callNumber, passNumber){
		...
		var str = newWord.word.match(/\p{L}\p{M}*/ug)
	...
	GetPositions: function(newWord, callNumber, passNumber){   
		if ((this.CENTER_FIRST)&&
			...
			if (this.Grid.length > this.Grid[0].length){
				...
				y = Math.floor((this.Grid[0].length - strLength)/2);
				d = this.VERTICAL
			} else {
				x = Math.floor((this.Grid.length - strLength)/2);
	...
	TestPosition: function(newWord,x,y,direction){
		...
		var regex = new RegExp(/\p{L}\p{M}*/ug);
		var str = newWord.match(regex)
